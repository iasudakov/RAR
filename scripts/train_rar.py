...
def main():
    ...
    if accelerator.is_main_process:
        try:
            from yt_tools.nirvana_utils import copy_snapshot_to_out
            copy_snapshot_to_out(output_dir)
            logger.info("Restored snapshot to output dir.")
        except Exception:
            pass
    ...
    if accelerator.is_main_process:
        ...
        if metadata_path.exists():
            ...
            run = client.init_run(name=config.experiment.name)
        torch.save({"run_uid": run.uid}, metadata_path)
    else:
        run = None
    ...
    # Register extra stateful objects so accelerator.save_state/load_state covers them.
    if ema_model is not None:
        accelerator.register_for_checkpointing(ema_model)
    if loss_module is not None and len(list(loss_module.parameters())) > 0:
        accelerator.register_for_checkpointing(loss_module)
