...
def create_dataloader(config, logger, accelerator):
    ...
    # Pretokenized JSONL dataset (local file).
    if dataset_config.get("pretokenization", "").endswith(".jsonl"):
    ...
    elif dataset_config.get("pretokenization", "") and dataset_config.get("dataset_with_text_label", False) is True:
...
def auto_resume(config, logger, accelerator, ema_model,
    ...
    if config.experiment.resume:
        ...
        # Prefer checkpoint-last (always the most recent complete save).
        last_path = Path(config.experiment.output_dir) / "checkpoint-last"
        if last_path.exists():
            checkpoint_path = last_path
        else:
            numbered = [p for p in glob.glob(os.path.join(
                config.experiment.output_dir, "checkpoint-[0-9]*"))]
            if numbered:
                checkpoint_path = Path(sorted(numbered,
                    key=lambda x: int(x.split('-')[-1]), reverse=True)[0])
            else:
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.log_every == 0:
                ...
                accelerator.log(logs, step=global_step + 1)
                if run is not None:
                    run.log({"train_loss": gen_logs["train/loss"],
                             "train_accuracy": gen_logs["train/correct_tokens"],
                             "lr": lr}, step=global_step + 1)
...
def save_checkpoint(model, output_dir, accelerator, global_step, logger) -> Path:
    ...
    # Numbered checkpoint: model weights only (saves disk space).
    ...
    # checkpoint-last: full training state for resume (overwrites previous).
    last_path = Path(output_dir) / "checkpoint-last"
